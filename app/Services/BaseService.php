...
abstract class BaseService
{
    ...
    {
        $rejection = AdConfirmationEnum::REJECTED->value;
        $confirmation = AdConfirmationEnum::CONFIRMED->value;
        $acceptBtn = "<button class='btn btn-success accept-btn' data-status='{$confirmation}'   name='acceptBtn'  data-accept-id='{$obj->id}' >
         " . ($obj->{$column} == AdConfirmationEnum::CONFIRMED->value ? trns('accepted') : trns('accept')) . "
            </button>";

        $acceptBtnDisabled = "<button disabled class='btn btn-success' data-accept-id='{$obj->id}'>
            " . trns('accepted') . "
               </button>";
        //dd($obj->id);
        $rejectBtn = "<button class='btn btn-danger refuse-btn' data-refuse-id='{$obj->id}' data-status='{$rejection}' name='rejectBtn'   data-bs-toggle='modal' data-bs-target='#refuse_reason_modal'>
    ...
    public function storeFiles($files, $folder = null)
    {
        ...
        foreach ($files as $file) {
            ...
            $relativeFilePath = $folder . '/' . uniqid() . '_' . time() . '.' . $extension;

            $file->move($storagePath, basename($relativeFilePath));

            $storedFiles[] = [
                'file_path' => 'storage/' . $relativeFilePath,
                'file_extension' => $extension,
                'file_name' => $nameWithoutExt
            ];
        }

        return count($storedFiles) === 1 ? $storedFiles[0] : $storedFiles;
    }
